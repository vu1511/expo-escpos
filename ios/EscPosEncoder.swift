...
class EscPosEncoder {
    ...
    init(options: [String: Any] = [:]) {
        ...
        self.embedded = (self.options["width"] as? Int) != nil && (self.options["embedded"] as? Bool) == true
        self.buffer = []
        self.queued = []
        self.cursor = 0
        self.codepage = "ascii"
    ...
    private func encode(_ value: String) -> [UInt8] {
        ...
        let fragments = CodePageEncoder.autoEncode(input: value, candidates: self.options["codepageCandidates"] as? [String] ?? [])
        ...
        for fragment in fragments {
            ...
            result.replaceSubrange((index + 3)..<(index + 3 + fragment.bytes.count), with: fragment.bytes)
        ...
        if self.embedded {
            ...
            if self.state["align"] as? String == "center" {
                ...
                if indent + remainder > 0 {
                    self.queued.insert(contentsOf: Array(repeating: 0x20, count: indent + remainder), at: 0)
                }
            }
            
            if self.state["align"] as? String == "right" {
                self.queued.insert(contentsOf: Array(repeating: 0x20, count: indent), at: 0)
            }
        }
    ...
    private func wrap(_ value: String, position: Int? = nil) -> [String] {
       ...
       if position == nil && (!(options["wordWrap"] as? Bool ?? true) || options["width"] == nil) {
           return [value]
       }
    ...
    func text(_ value: String, wrap: Int? = nil) -> EscPosEncoder {
        ...
        for (index, line) in lines.enumerated() {
            ...
            if (self.options["width"] as? Int) == 46 && (self.state["align"] as? String) != "center" {
                // k80 margin 2 space
    ...
    func pulse(_ device: Int = 0, on: Int = 100, off: Int = 500) -> EscPosEncoder {
        ...
        self.queue([0x1b, 0x70, device != 0 ? 1 : 0, UInt8(onValue & 0xff), UInt8(offValue & 0xff)])
